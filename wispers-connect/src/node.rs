...
impl RegisteredNode {
    ...
    pub async fn logout(&mut self) -> Result<(), NodeStateError> {
        ...
        match client.deregister_node(&self.registration).await {
            Ok(()) => {}
...
impl ActivatedNode {
    ...
    pub async fn logout(&mut self) -> Result<(), NodeStateError> {
        ...
        let revocation_payload = build_revocation_payload(
            &new_roster,
            self.registration.node_number, // revoked
            self.registration.node_number, // revoker (self)
        );
        ...
        match client.update_roster(&self.registration, new_roster).await {
            Ok(_) => {
...
impl Node {
    ...
    pub async fn group_info(&self) -> Result<GroupInfo, NodeStateError> {
        ...
        let nodes: Vec<NodeInfo> = hub_nodes
            ...
            .map(|hub_node| {
                let is_activated =
                    if is_dead_roster || (roster.version == 0 && activated_set.is_empty()) {
                        Some(false)
    ...
    pub async fn logout(&mut self) -> Result<(), NodeStateError> {
        ...
        match self {
            Node::Pending(p) => p.logout().await,
            Node::Registered(r) => r.logout().await,
            Node::Activated(a) => a.logout().await,
            Node::Placeholder => unreachable!(),
        }?;

        // All hub + store operations succeeded — transition to Pending.
        let node = std::mem::replace(self, Node::Placeholder);
        match node {
            Node::Pending(p) => *self = Node::Pending(p),
            Node::Registered(r) => {
                *self = Node::Pending(PendingNode::new(r.persisted, r.store, r.config));
            }
            Node::Activated(a) => {
                *self = Node::Pending(PendingNode::new(a.persisted, a.store, a.config));
            }
            Node::Placeholder => unreachable!(),
        }
