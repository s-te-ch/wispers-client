...
async fn test_p2p_connection_via_hub() {
    ...
    // Node 2 receives the incoming connection
    let answerer_conn = incoming_rx.recv().await.expect("node2 receives connection");
...
async fn test_p2p_multiple_messages() {
    ...
    let answerer = incoming_rx.recv().await.expect("receives connection");
